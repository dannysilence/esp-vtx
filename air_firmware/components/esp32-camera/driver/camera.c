...
static void IRAM_ATTR signal_dma_buf_received(bool* need_yield, bool last)
{
...
static void IRAM_ATTR dma_filter_buffer(size_t buf_idx, bool last)
{
...

    {
        ...
        size_t stride = sizeof(dma_elem_t);
        s_cb(src, stride, count, last);
    }
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    ESP_LOGD(TAG, "in_bpp: %d, fb_bpp: %d, mode: %d, width: %d height: %d",
...
esp_err_t esp_camera_init(const camera_config_t* config, camera_fb_data_cb_t cb)
{
